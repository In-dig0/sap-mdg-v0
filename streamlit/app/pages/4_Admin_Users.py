...
def validate_password(pwd: str) -> list[str]:
    """Ritorna lista di errori. Lista vuota = password valida."""
    errors = []
    if len(pwd) < 8:
...
with st.expander("➕ Crea nuovo utente", expanded=False):
    with st.form("new_user_form"):
        ...
        new_role     = col2.selectbox("Ruolo", ["business_role", "it_role", "admin_role"])
    ...
    if submitted:
        ...
        if r.status_code == 201:
            ...
            st.success(f"Utente **{new_email}** creato con ruolo `{new_role}`.")
            st.cache_data.clear()
        else:
...
if not users:
    ...
    # Intestazione — senza colonna Azione
    h1, h2, h3, h4, h5 = st.columns([2, 3, 2, 1, 1])
    h1.markdown("**Nome**")
    h2.markdown("**Email**")
    h3.markdown("**Ruolo**")
    h4.markdown("**Stato**")
    h5.markdown("**Modifica**")
    st.divider()
    ...
    for u in users:
        ...
        if c5.button("✏️", key=f"btn_{u['id']}", help="Modifica utente",
        ...
        if st.session_state.get(edit_key, False):
            with st.container(border=True):
                ...
                new_role_val = ef1.selectbox(
                    "Ruolo",
                    options=["business_role", "it_role", "admin_role"],
                    index=["business_role", "it_role", "admin_role"].index(
                        u.get("role", "business_role")
                    ),
                    key=f"role_{u['id']}",
                )
                ...
                if bs.button("💾 Salva", key=f"save_{u['id']}",
                             use_container_width=True):
                    errors = []
                    # Valida password se compilata
                    if new_pwd:
                ...
                if bc.button("✖ Annulla", key=f"cancel_{u['id']}",
                             use_container_width=True):
                    st.session_state[edit_key] = False
                    st.rerun()

# Messaggio di esito positivo — mostrato in fondo alla pagina
