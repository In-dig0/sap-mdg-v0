...
# =============================================================================
# Query Q0 — solo A2F, senza join (tutti gli articoli dell'archivio)
# =============================================================================
@st.cache_data(ttl=300)
def load_a2f_only(
    suffix: str,
...
# =============================================================================
# Query Q2 — articoli con loghi, con filtri dinamici (cached)
# =============================================================================
@st.cache_data(ttl=300)
def load_q2(
...
modalita = st.radio(
    "Modalità",
    options=["Solo archivio (A2F)", "Con vendite e loghi", "Solo loghi multipli"],
    index=1,
    horizontal=True,
    help=(
        "**Solo archivio**: tutti gli articoli di A2F senza join — "
        "corrisponde esattamente al totale della tabella. "
        "**Con vendite e loghi**: join con PUNTFOR e CLIM, include tutti. "
        "**Solo loghi multipli**: solo articoli con N_LOGHI > 1."
    ),
)
...
try:
    with st.spinner("Caricamento dati..."):
        if modalita == "Solo archivio (A2F)":
            df_q2 = load_a2f_only(
                plant_suffix, t_tipo_prod, t_flag6, t_fantasma, t_codart
            )
            df_q2["N_LOGHI"] = None
        else:
            solo_multipli = (modalita == "Solo loghi multipli")
            df_q2 = load_q2(
...
if modalita == "Solo archivio (A2F)":
    ...
    kpi_multi_art    = 0
    kpi_logo_singolo = kpi_articoli
    kpi_loghi        = 0
    kpi_normalizzati = kpi_articoli
else:
    kpi_articoli     = len(df_q2)
...
with col_title_q2:
    if modalita == "Solo archivio (A2F)":
        titolo = f"📋 Archivio articoli — Plant {plant_label}"
    elif modalita == "Solo loghi multipli":
        titolo = f"📋 Articoli con loghi multipli — Plant {plant_label}"
    else:
        titolo = f"📋 Articoli per logo — Plant {plant_label}"
    st.subheader(titolo)
...
if modalita == "Solo archivio (A2F)":
    st.caption(f"**{kpi_articoli} articoli** in archivio")
else:
    st.caption(f"**{kpi_articoli} articoli** totali — **{kpi_multi_art}** con loghi multipli")

with col_export_q2:
    st.download_button(
        label="⬇️ Esporta",
        data=to_csv_bytes(df_q2),
        file_name=f"targhette_{plant_suffix}_{date_from}.csv",
        mime="text/csv",
...
if modalita != "Solo archivio (A2F)":
    col_config["N_LOGHI"] = st.column_config.NumberColumn("# Loghi", format="%d")
