...
SELECT
    ...
    END                                          AS message,
    CASE
        WHEN raw."ZWELS_01" IS NULL OR raw."ZWELS_01" = ''  THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK009')
        WHEN NOT EXISTS (
            SELECT 1 FROM ref."SAP_Mod_Pagamento" ref
            WHERE ref."Cod_Mod_Pag" = raw."ZWELS_01"
        )                                                    THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK009')
        ELSE 'Ok'
    END                                          AS status,
