""" @bruin
name: stg.ck801_customer_vat_vies
type: python
depends:
  - stg.clean_check_results
description: >
  CK801 — EXT_REF: Clienti: verifica validità Partita IVA EU/UK
  tramite servizi esterni (VIES SOAP + HMRC API v2).
  Legge da raw."S_CUST_TAXNUMBERS#ZBP-CodiciFisc" i record con
  ...
  Il check CK802 legge questa tabella e scrive la sintesi in
...
log = logging.getLogger("ck801")

# ---------------------------------------------------------------------------
# Costanti
# ---------------------------------------------------------------------------
CHECK_ID     = "CK801"
SOURCE_TABLE = "S_CUST_TAXNUMBERS#ZBP-CodiciFisc"
CATEGORY     = "BP"
...
def check_single(
    ...
    if not is_eu and not is_gb:
        ...
        log.debug(f"  [customer:{entity_id}] {vat_raw} → NOT_EU")
        return base
    ...
    if is_eu:
        try:
            ...
            log.info(
                f"  [customer:{entity_id}] {vat_raw} → {base['check_status']} (VIES)"
                + (f" | {resp['name']}" if resp["name"] else "")
            )
        except Fault as e:
            base["check_status"]  = "ERROR"
            base["error_message"] = f"SOAP Fault: {e.message}"
            log.warning(f"  [customer:{entity_id}] {vat_raw} → SOAP Fault: {e.message}")
        except Exception as e:
            base["check_status"]  = "ERROR"
            base["error_message"] = str(e)
            log.warning(f"  [customer:{entity_id}] {vat_raw} → ERROR VIES: {e}")
        return base
    ...
    if is_gb:
        if not hmrc.is_available():
            ...
            log.warning(f"  [customer:{entity_id}] {vat_raw} → GB_NO_CREDENTIALS")
            return base
        try:
            ...
            log.info(
                f"  [customer:{entity_id}] {vat_raw} → {base['check_status']} (HMRC)"
                + (f" | {resp['name']}" if resp["name"] else "")
            )
        except requests.exceptions.HTTPError as e:
            base["check_status"]  = "ERROR"
            base["error_message"] = f"HMRC HTTP {e.response.status_code}: {e.response.text[:200]}"
            log.warning(f"  [customer:{entity_id}] {vat_raw} → HMRC ERROR: {e}")
        except Exception as e:
            base["check_status"]  = "ERROR"
            base["error_message"] = str(e)
            log.warning(f"  [customer:{entity_id}] {vat_raw} → ERROR HMRC: {e}")
        return base
...
def main():
    ...
    try:
        with conn.cursor() as cur:
            ...
            #     per ogni (entity_id, vat_number) customer teniamo solo il record
            ...
            conn.commit()
            if cleaned:
                log.info(f"Rimossi {cleaned} record CACHED customer residui da run precedenti.")

            # 3. Leggi VAT customer da raw
            cur.execute("""
                SELECT
                    "KUNNR(k/*)"   AS entity_id,
        ...
        if invalids:
            ...
            for r in invalids:
                print(f"       customer:{r['entity_id']} → {r['vat_number']}")
