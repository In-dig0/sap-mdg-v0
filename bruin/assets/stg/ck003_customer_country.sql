...
SELECT
    ...
    END                                          AS message,
    CASE
        WHEN raw."COUNTRY(*)" IS NULL OR raw."COUNTRY(*)" = ''  THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK003')
        WHEN NOT EXISTS (
            SELECT 1 FROM ref."SAP_EXPORT_T005S" ref
            WHERE ref."LAND1" = raw."COUNTRY(*)"
        )                                                        THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK003')
        ELSE 'Ok'
    END                                          AS status,
