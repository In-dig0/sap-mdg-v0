...
description: >
  ...
  In SAP questo può causare errori di deduplicazione BP.
...
JOIN (
    -- Trova le coppie TAXTYPE+TAXNUM presenti su più LIFNR distinti
    SELECT
        "TAXTYPE(k/*)",
        "TAXNUM(*)",
...
WHERE t."TAXNUM(*)" IS NOT NULL
  AND t."TAXNUM(*)" <> ''
ORDER BY t."TAXTYPE(k/*)", t."TAXNUM(*)", t."LIFNR(k/*)"
;
