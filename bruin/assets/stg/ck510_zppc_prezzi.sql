/* @bruin
name: stg.ck510_zppc_prezzi
type: pg.sql
depends:
  - stg.clean_check_results
description: >
  CK510 — CROSS_SOURCE: Pricing vendite (FA-ZPPC-PrezziPianiConsegna).
  Tre verifiche:
...
WITH results AS (
    ...
    SELECT
        ...
        CASE
            WHEN raw."Material" IS NULL OR raw."Material" = ''
                THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK510')
            WHEN NOT EXISTS (
                SELECT 1 FROM raw."S_MVKE" mvke
                WHERE mvke."PRODUCT(k/*)" = raw."Material"
            )
    ...
    FROM raw."FA-ZPPC-PrezziPianiConsegna" raw
...
SELECT
    'FA-ZPPC-PrezziPianiConsegna'                AS source_table,
    'MAT'                                        AS category,
    r.object_key,
    'CK510'                                      AS check_id,
    r.message,
...
WHERE (
    ...
    FROM stg.check_catalog WHERE check_id = 'CK510'
)
;
