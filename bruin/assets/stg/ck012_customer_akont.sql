...
SELECT
    ...
    END                                          AS message,
    CASE
        WHEN raw."AKONT(*)" IS NULL OR raw."AKONT(*)" = ''  THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK012')
        WHEN NOT EXISTS (
            SELECT 1 FROM ref."SAP_Conto_Riconciliazione_Clienti" ref
            WHERE ref."Cod_Conto" = raw."AKONT(*)"
        )                                              THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK012')
        ELSE 'Ok'
    END                                          AS status,
