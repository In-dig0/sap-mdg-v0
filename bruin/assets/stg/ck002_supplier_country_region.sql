/* @bruin
name: stg.ck002_supplier_country_region
type: pg.sql
depends:
  - stg.clean_check_results
description: >
  CK002 — SAP_REF: Fornitori: coppia COUNTRY+REGION presente in T005S.
...
SELECT
    ...
    END                                          AS message,
    CASE
        WHEN raw."REGION" IS NULL OR raw."REGION" = ''  THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK002')
        WHEN NOT EXISTS (
            SELECT 1 FROM ref."SAP_EXPORT_T005S" ref
            WHERE ref."LAND1" = raw."COUNTRY"
              AND ref."BLAND" = raw."REGION"
        )                                               THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK002')
        ELSE 'Ok'
    END                                          AS status,
...
WHERE (
    ...
    FROM stg.check_catalog WHERE check_id = 'CK002'
)
;
