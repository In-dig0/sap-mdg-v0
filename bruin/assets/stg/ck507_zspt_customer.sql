/* @bruin
name: stg.ck507_zspt_customer
type: pg.sql
depends:
  - stg.clean_check_results
description: >
  CK507 — CROSS_SOURCE: Pricing vendite (BO-ZSPT-ScontoTestataCliente):
...
SELECT
    'BO-ZSPT-ScontoTestataCliente'               AS source_table,
    'MAT'                                        AS category,
    ...
    CASE
        ...
        AND NOT EXISTS (
            ...
            THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK507')
        ELSE 'Ok'
    END                                          AS status,
    ...
    NOW()                                        AS created_at
FROM raw."BO-ZSPT-ScontoTestataCliente" raw
WHERE
    raw."Customer" IS NOT NULL AND raw."Customer" <> ''
    AND (
        SELECT COALESCE(is_active, FALSE)
        FROM stg.check_catalog WHERE check_id = 'CK507'
    )
;
