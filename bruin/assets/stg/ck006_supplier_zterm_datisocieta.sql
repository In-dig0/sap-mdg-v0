/* @bruin
name: stg.ck006_supplier_zterm_datisocieta
type: pg.sql
depends:
  - stg.clean_check_results
description: >
  CK006 — SAP_REF: Fornitori (ZBP_DatiSocieta): campo ZTERM1
...
SELECT
    ...
    END                                          AS message,
    CASE
        WHEN raw."ZTERM1" IS NULL OR raw."ZTERM1" = ''  THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK006')
        WHEN NOT EXISTS (
            SELECT 1 FROM ref."SAP_EXPORT_T052" ref
            WHERE ref."ZTERM" = raw."ZTERM1"
        )                                               THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK006')
        ELSE 'Ok'
    END                                          AS status,
...
WHERE (
    ...
    FROM stg.check_catalog WHERE check_id = 'CK006'
)
;
