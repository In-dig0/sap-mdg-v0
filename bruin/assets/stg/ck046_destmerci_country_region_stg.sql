/* @bruin
name: stg.ck046_destmerci_country_region_stg
type: pg.sql
depends:
  - stg.clean_check_results
  - stg.detect_new_records  
description: >
  CK046 — SAP_REF: Destinatari Merci STG: coppia COUNTRY(*)+REGION presente in T005S.
...
SELECT
    ...
    END                                          AS message,
    CASE
        WHEN raw."REGION" IS NULL OR raw."REGION" = ''  THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK046')
        WHEN NOT EXISTS (
            SELECT 1 FROM ref."SAP_EXPORT_T005S" ref
            WHERE ref."LAND1" = raw."COUNTRY(*)"
              AND ref."BLAND" = raw."REGION"
        )                                               THEN (SELECT severity FROM stg.check_catalog WHERE check_id = 'CK046')
        ELSE 'Ok'
    END                                          AS status,
...
WHERE (
    ...
    FROM stg.check_catalog WHERE check_id = 'CK046'
)
;
