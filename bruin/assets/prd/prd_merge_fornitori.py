""" @bruin
name: prd.merge_fornitori
type: python
description: >
  Merge raw + stg → prd."S_SUPPL_GEN#ZBP_DatiGenerali".
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                ...
                merge_table(
                    cur,
                    raw_schema="raw", raw_table="S_SUPPL_GEN#ZBP_DatiGenerali",
                    stg_schema="stg", stg_table="S_SUPPL_GEN#ZBP_DatiGenerali_STG",
                    prd_table="S_SUPPL_GEN#ZBP_DatiGenerali",
                )
    finally:
        conn.close()
    log.info("=== prd.merge_fornitori completato ===")
